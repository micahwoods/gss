...
nh4_plot_sim <- p + theme_cowplot() +
  ...
  labs(x = expression(paste('mg N', H[4], '–N ', kg^{-1}))) 
#  labs(x = expression(paste('soil organic matter, g ', kg^{-1})))

nh4_plot_sim

d_na_nh4 <- subset(d, NH4N > 0)

# plot with sim vs gss actual
p <- ggplot(data = d_na_nh4, aes(x = NH4N))
nh4_plot_survey <- p + theme_cowplot() +
  background_grid(major = 'x') +
