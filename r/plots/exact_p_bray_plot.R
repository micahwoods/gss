...
# bray 2 plot with sim data
p <- ggplot(data = m_values, aes(x = x.values, y = y.values))
bray_plot_sim <- p + theme_cowplot() +
  ...
  labs(x = expression(paste('Bray 2, mg P ', kg^{-1}))) 
#  labs(x = expression(paste('soil organic matter, g ', kg^{-1})))

bray_plot_sim

# p plot with sim vs gss actual
p <- ggplot(data = d, aes(x = PB2))
bray_plot_survey <- p + theme_cowplot() +
  background_grid(major = 'x') +
