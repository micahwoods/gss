### POTASSIUM
# now get to the more standard analysis
...
k_plot_sim <- p + theme_cowplot() +
  ...
  labs(x = expression(paste('mg K ', kg^{-1}))) 
#  labs(x = expression(paste('soil organic matter, g ', kg^{-1})))

k_plot_sim

# k plot with sim vs gss actual
p <- ggplot(data = d, aes(x = KM3))
k_plot_survey <- p + theme_cowplot() +
  background_grid(major = 'x') +
