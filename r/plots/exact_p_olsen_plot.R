...
# olsen plot with sim data
p <- ggplot(data = m_values, aes(x = x.values, y = y.values))
olsen_plot_sim <- p + theme_cowplot() +
  ...
  labs(x = expression(paste('Olsen, mg P ', kg^{-1}))) 
#  labs(x = expression(paste('soil organic matter, g ', kg^{-1})))

olsen_plot_sim

# p plot with sim vs gss actual
p <- ggplot(data = d, aes(x = PO))
olsen_plot_survey <- p + theme_cowplot() +
  background_grid(major = 'x') +
